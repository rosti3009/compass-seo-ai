...
def _content_article_publishing_metadata(value: str | None) -> dict[str, object]:
    """Return article publishing metadata with backward-compatible optional defaults.

    Older article drafts may have an empty object, malformed JSON, or only a subset of
    the newer ISTORE publishing-package fields. Dashboard/API consumers should be
    able to read the same keys for every draft without assuming a package was
    generated by the latest article engine.
    """
    metadata = _dict_or_empty(_json_load(value, {})).copy()
    istore_package = _dict_or_empty(metadata.get("istore_copy_paste_package")).copy()
    image_package = _list_or_empty(metadata.get("image_package"))
    placement_guide = _list_or_empty(metadata.get("image_placement_guide"))
    istore_steps = _list_or_empty(
        metadata.get("istore_copy_paste_steps") or istore_package.get("steps")
    )
    article_blocks = _list_or_empty(
        metadata.get("article_blocks")
        or metadata.get("istore_block_structure")
        or istore_package.get("article_blocks")
    )
    visual_elements = _dict_or_empty(metadata.get("visual_elements"))
    publishing_qa = _dict_or_empty(
        metadata.get("publishing_qa") or metadata.get("final_qa_validation")
    )

    istore_package.setdefault("mode", "ISTORE_COPY_PASTE")
    istore_package["steps"] = istore_steps
    istore_package["article_blocks"] = article_blocks

    metadata.update(
        {
            "image_package": image_package,
            "image_placement_guide": placement_guide,
            "istore_copy_paste_package": istore_package,
            "istore_copy_paste_steps": istore_steps,
...
class ContentArticleDraft(Base):
    ...
    def to_dict(self) -> dict[str, object]:
        publishing_metadata = _content_article_publishing_metadata(self.image_generation_metadata_json)
        return {
            "id": self.id,
            "status": self.status,
            ...
            "image_generation_metadata": publishing_metadata,
            "image_package": publishing_metadata["image_package"],
            "image_placement_guide": publishing_metadata["image_placement_guide"],
            "istore_copy_paste_steps": publishing_metadata["istore_copy_paste_steps"],
            "article_blocks": publishing_metadata["article_blocks"],
            "visual_elements": publishing_metadata["visual_elements"],
            "publishing_qa": publishing_metadata["publishing_qa"],
