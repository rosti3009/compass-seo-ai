...
def publish_approved_fix(
    ...
    *,
    ...
    if dry_run:
         _append_publish_log(approval, "Dry-run publish validation passed; no ISTORE PUT sent")
        db.add(approval)
        db.commit()
        db.refresh(approval)
        return {
    ...
    client = client or IStoreClient.from_settings()
    try:
        _append_publish_log(approval, f"Sending SEO-only ISTORE update for {approval.field_path}")
    ...
    if _field_value(fetched, approval.field_path) == approval.proposed_value:
        ...
        approval.status = "FAILED_REVIEW_REQUIRED"
        verified = False
        _append_publish_log(approval, "ISTORE update response received, but verification did not match proposed value")
    db.add(approval)
    db.commit()
    db.refresh(approval)
...
def rollback_published_fix(
    ...
    *,
    ...
    if dry_run:
        ...
        db.commit()
        db.refresh(approval)
        return {"success": True, "dry_run": True, "put_sent": False, "rollback_preview": rollback_preview(approval)}
    if not settings.istore_publish_enabled:
    ...
    client = client or IStoreClient.from_settings()
    try:
        _append_publish_log(approval, f"Sending SEO-only ISTORE rollback for {approval.field_path}")
        response = client.update_product(approval.target_id, payload)
        approval.publish_response_json = json.dumps(_redacted_json(response), ensure_ascii=False)
        fetched = client.get_product(approval.target_id)
    except IStoreAPIError as exc:
    ...
    if _field_value(fetched, approval.field_path) == (approval.current_value or ""):
        ...
        approval.status = "ROLLBACK_FAILED_REVIEW_REQUIRED"
        verified = False
        _append_publish_log(approval, "Rollback response received, but verification did not match original value")
    db.add(approval)
    db.commit()
    db.refresh(approval)
    return {"success": verified, "verified": verified, "put_sent": True, "fix": approval.to_dict()}
...
def preview_generated_content(approval: IStoreSEOApproval, db: Session | None = None) -> dict[str, object]:
    ...
    article = stored_preview.get("hebrew_article") or _hebrew_article(
        product_name, category, _clean(approval.proposed_value)
    )
    ...
    internal_links = stored_preview.get("internal_links") or _internal_link_suggestions(
        approval.target_url, product_name, pages
    )
    schema = stored_preview.get("schema_suggestions") or _schema_suggestions(
        approval.target_type, product_name, approval.target_url, faq
    )
    return {
        "fix_id": approval.id,
        "status": approval.status,
...
def _product_fix_proposals(product: dict[str, Any], pages: list[PageAudit] | None = None) -> list[ProposedFix]:
    ...
    if has_weak_title:
        proposals.append(
            ProposedFix("meta_title", meta_title, suggested_title, "כותרת SEO חסרה, קצרה או לא ממוקדת בעברית")
        )
    ...
    if not keyword or len(keyword) < 4 or " " in keyword.strip("/"):
        proposals.append(ProposedFix("keyword", keyword, _slugify(name), "הצעת URL keyword/slug בטוחה לשדה SEO בלבד"))
    return proposals


def _site_page_content_opportunities(db: Session, pages: list[PageAudit] | None = None) -> list[IStoreSEOApproval]:
    ...
    for page in pages:
        ...
        approval = IStoreSEOApproval(
            ...
            rollback_payload_json=json.dumps({}, ensure_ascii=False),
             ...
             _append_publish_log(
            approval, "Hebrew article/category draft created for preview only; no ISTORE payload generated"
            )
...
def _expanded_description(
    name: str, current: str, category: str, target_url: str | None, pages: list[PageAudit]
) -> str:
...
def _hebrew_article(topic: str, category: str, context: str = "") -> str:
    ...
    return (
        ...
        f"<h2>איך לבחור נכון?</h2>"
        f"<ul><li>בדקו שהמפרט מתאים לצורך האמיתי שלכם.</li><li>קראו את תיאור המוצר וההנחיות לפני שימוש.</li>"
        f"<li>העדיפו מוצר עם מידע ברור, תמונות איכותיות ואפשרות לקבל תמיכה.</li></ul>"
        f"<p>צוות Compass ממליץ לבחור לפי שימוש, אמינות וחוויית קנייה מלאה - לא רק לפי כותרת קצרה.</p>"
        f"</article>"
    )
...
def _hebrew_faq(name: str, category: str) -> list[dict[str, str]]:
    ...
    return [
        ...
        },
        {
            ...
            "answer": "מומלץ לבדוק התאמה לשימוש הרצוי, מפרט, מידות או תאימות, תנאי משלוח וזמינות תמיכה לאחר הקנייה.",
        },
        {
...
def _schema_suggestions(target_type: str, name: str, url: str | None, faq: object) -> list[dict[str, object]]:
    ...
    article_schema = {"@context": "https://schema.org", "@type": "Article", "headline": name, "inLanguage": "he-IL"}
    if url:
        article_schema["mainEntityOfPage"] = url
    schemas = [faq_schema, article_schema]
    if target_type == "product":
        product_schema = {"@context": "https://schema.org", "@type": "Product", "name": name}
        if url:
...
def _internal_link_suggestions(
    target_url: str | None, topic: str, pages: list[PageAudit], limit: int = 5
) -> list[dict[str, str]]:
    ...
    for page in pages:
        ...
        suggestions.append(
            {"url": page.url, "anchor_text": label[:80], "reason": reason, "score": str(round(score, 1))}
        )
